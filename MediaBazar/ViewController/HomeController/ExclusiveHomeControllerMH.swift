//
//  ExclusiveHomeControllerMH.swift
//  MediaBazar
//
...
class ExclusiveHomeControllerMH: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        getAllStoryData(page: "0", key: "Exclusive", header: self.currenUserLogin.mediahouseToken)
    }
        ...
        WebService3.sharedInstance.AddToFavoriteStroy(storyId: storyId, storyHeader: header) { (result, message, response) in
            ...
            if result == 200{
                ...
                if self.isSearching == true {
                    self.getAllStoryData(page: "0", key: "Exclusive", header: self.currenUserLogin.mediahouseToken)
                }else {
                    self.getSearchData(page: "0", key: "Exclusive", searchKey: self.searchText, header: self.currenUserLogin.mediahouseToken)
                }
            }else{
                if self.isSearching == true {
                    self.getAllStoryData(page: "0", key: "Exclusive", header: self.currenUserLogin.mediahouseToken)
...
//-----TableView-----
extension ExclusiveHomeControllerMH: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ExclusiveHomeTableViewCell") as! ExclusiveHomeTableViewCell
        if isSearching {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if (totalPaginationPage - 3) == indexPath.row {
            ...
            if self.typeOfData == "exclusive"{
                getAllStoryData(page: "\(page)", key: "Exclusive", header: currenUserLogin.mediahouseToken)
            }else if typeOfData == "filter"{
                getFilterStoryData(page: "\(page)", key: "Exclusive", categoryId: self.getFilterID, header: currenUserLogin.mediahouseToken)
            }else if typeOfData == "search"{
                getSearchData(page: "\(page)", key: "Exclusive", searchKey: self.searchText, header: currenUserLogin.mediahouseToken)
            }else {
                return
            }
...
extension ExclusiveHomeControllerMH: UISearchBarDelegate {
    //----------- SearchBar Delegate --------------
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        if searchBar.text != "" && searchBar.text != nil {
            ...
            self.page = 0
            self.totalPages = 0
            getSearchData(page: "0", key: "Exclusive", searchKey: searchBar.text!, header: self.currenUserLogin.mediahouseToken)
        } else {
            isSearching = true
            self.storyList.removeAll()
            self.page = 0
...
//------getting Filter data
extension ExclusiveHomeControllerMH: SendNameOfAreaFilter {
    func FilterName(name: String, id: String) {
        if name == "clearFilter" {
            ...
            self.getFilterID = id
            self.getFilterStoryData(page: "0", key: "Exclusive", categoryId: id, header: self.currenUserLogin.mediahouseToken)
        }
    }
}
