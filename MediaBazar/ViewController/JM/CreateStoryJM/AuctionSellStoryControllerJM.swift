//
//  AuctionSellStoryControllerJM.swift
//  MediaBazar
//
...
class AuctionSellStoryControllerJM: UIViewController {
    
    @IBOutlet weak var acutionSellStoryTableView : UITableView!
    ...
    @IBOutlet weak var currancyTextField : UITextField!
    ...
    var categoryID = ""
    var currencyCode = ""
    var auctionStorydata = storyModal()
...
//        self.tabBarController?.tabBar.isHidden = true
        ...
        currancyTextField.delegate = self
    ...
    func setupButton(){
        continueButton.addTarget(self, action: #selector(onClickContnueButton), for: .touchUpInside)
    ...
    func setUpData(){
        ...
        self.currancyTextField.text = self.currentUserLogin.prevJouralistData.country.currencyName
        self.dateTextField.text = self.currentDate
        self.languageTextField.text = currentUserLogin.language
    }
    ...
    @objc func onClickContnueButton(){
        ...
        guard let duration = durationTextField.text, duration != "" else {
            NKToastHelper.sharedInstance.showErrorAlert(self, message: "Please enter the auction duration")
            return
        }
        
        guard let currency = currancyTextField.text, currency != "" else {
            NKToastHelper.sharedInstance.showErrorAlert(self, message: "Please enter the currency")
            return
        }
        
        guard let biddingPrice = priceTextField.text, biddingPrice != "" else {
            NKToastHelper.sharedInstance.showErrorAlert(self, message: "Please enter the auction bidding price")
            return
        }
        
        guard keywordList.count > 0 else {
            NKToastHelper.sharedInstance.showErrorAlert(self, message: "Please enter the keeyword")
            return
        }
        ...
        auctionSellStory(headline: headlineTextField.text!, categoryID: categoryID, languageCode: languageTextField.text!, country: countryData, state: stateData, city: cityData, date: dateTextField.text!, stepCount: "1", auctionDuration: durationTextField.text!, keywordID: keyWordDataId, storyCategory: "Auction", currency: currencyCode, biddingPrice: priceTextField.text!, descri: textView.text!, header: currentUserLogin.token)
...
//        self.navigationController?.pushViewController(postVC, animated: true)
    ...
    func auctionSellStory(headline: String, categoryID: String, languageCode: String, country: String, state: String, city: String, date: String, stepCount: String, auctionDuration: String, keywordID: String, storyCategory: String, currency: String, biddingPrice: String, descri: String,header: String){
        Webservices.sharedInstance.auctionSellStrotyForm(headLine: headline, categoryId: categoryID, languageCode: languageCode, country: country, state: state, city: city, date: date, stepCount: stepCount, auctionDuartion: auctionDuration, keyWordId: keywordID, storyCategory: storyCategory, currency: currency, biddingPrice: biddingPrice, description: descri, header: header){
            (result,message,response) in
            if result == 200{
                print(response)
                if let somecategory = response{
                    self.auctionStorydata = somecategory
                    print("======================\(somecategory)")
                }
                let PostStoryTypeVC = AppStoryboard.Journalist.viewController(PostStoryTypeViewControllerJM.self)
                PostStoryTypeVC.blogId = self.auctionStorydata.blogID
...
extension AuctionSellStoryControllerJM: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
extension AuctionSellStoryControllerJM: UITextFieldDelegate {
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        } else if textField == currancyTextField{
...
extension AuctionSellStoryControllerJM : SendNameOfLanguage{
...
extension AuctionSellStoryControllerJM: SendNameOfCountry {
...
extension AuctionSellStoryControllerJM: SendNameOfState {
...
//        self.currancyTextField.text = currencyName
        self.stateID = id
        self.stateData = id
        // convert Response into json Object
...
extension AuctionSellStoryControllerJM: SendNameOfCity {
...
extension AuctionSellStoryControllerJM : SendNameOfCategory{
...
extension AuctionSellStoryControllerJM: SendKeywordName{
...
extension AuctionSellStoryControllerJM: SendNameOfCurrency {
    func currencyName(name: String, id: String) {
        currancyTextField.resignFirstResponder()
        if name == "" && id == ""{
            return
        }
        self.currancyTextField.text = name
