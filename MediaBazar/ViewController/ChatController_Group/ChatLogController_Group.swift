...
class ChatLogController_Group: UIViewController {
    
    lazy var collectionView: UICollectionView = {
        let flowLayout = UICollectionViewFlowLayout()
        
        let cv = UICollectionView(frame: self.view.bounds, collectionViewLayout: flowLayout)
        cv.register(ChatMessageCell_Group.self, forCellWithReuseIdentifier: "collectionCell_Group")
    ...
    var messages = [Message_Group]()
    
    var user: Users_Group? {
        didSet {
...
//            observeOnlineStatus()
    ...
    func observeMessages() {
        guard let toId = user?.chatID, let uid = user?.senderID, let chatID = user?.chatID else {
            return
        }
        
        let messageIdRef = Database.database().reference().child("chat").child("groups").child(chatID)
    ...
    func observerChildedAdded() {
        ...
        messagesRef.observe(.childAdded) { (snap) in
            ...
            messRef.observeSingleEvent(of: .value, with: { (snapshot) in
                ...
                let tempMessage = Message_Group(dictionary: dictionary)
    ...
    func observeOnlineStatus() {
        ...
        //                self.statusLabel.text = values
        //            }
        //        }
        
        let statusRef = Database.database().reference().child("users").child((user?.chatID)!)
    ...
    lazy var topBar: UIView = {
        ...
        if let groupImageUrl = self.user?.groupImage, groupImageUrl != "" {
            groupImage.sd_setImage(with: URL(string: "\(self.baseUrl)\(groupImageUrl)"),placeholderImage: UIImage(named: "default_profile"), completed: nil)
        }
        groupImage.layer.cornerRadius = 18
        groupImage.layer.masksToBounds = true
        groupImage.contentMode = UIView.ContentMode.scaleAspectFill
        groupImage.translatesAutoresizingMaskIntoConstraints = false
        
        tb.addSubview(groupImage)
        
        groupImage.leftAnchor.constraint(equalTo: backButton.rightAnchor, constant: 5).isActive = true
        groupImage.centerYAnchor.constraint(equalTo: backButton.centerYAnchor, constant: 0).isActive = true
        //        groupImage.centerXAnchor.constraint(equalTo: tb.centerXAnchor, constant: 0).isActive = true
        //        nameLabel.rightAnchor.constraint(equalTo: tb.rightAnchor, constant: -5).isActive = true
        groupImage.widthAnchor.constraint(equalToConstant: 40).isActive = true
        groupImage.heightAnchor.constraint(equalTo: backButton.widthAnchor, multiplier: 1).isActive = true
        
        let nameLabel = UILabel()
        nameLabel.text = self.user?.groupName
        let titleFont = UIFont(name: "Lato-Regular", size: 20)
        ...
        nameLabel.leftAnchor.constraint(equalTo: groupImage.rightAnchor, constant: 15).isActive = true
        nameLabel.centerYAnchor.constraint(equalTo: groupImage.centerYAnchor, constant: 0).isActive = true
    ...
    @objc func handleSendButton() {
        ...
        if firstMessage {
            ...
            let messageIdRef = Database.database().reference().child("chat").child("groups")
        ...
        let toID = user!.chatID!
        let fromID = user!.senderID!
        let timestamp = ServerValue.timestamp()
        
        let values: [String: Any] = ["message" : inputTextField.text!, "fromId": fromID, "timestamp": timestamp, "mediaUrl": "", "messageType" : "text"]
...
extension ChatLogController_Group: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "collectionCell_Group", for: indexPath as IndexPath) as! ChatMessageCell_Group
    ...
    fileprivate func setupCell(_ cell: ChatMessageCell_Group, message: Message_Group) {
        ...
        if let messageByUser = message.fromId, let members = self.user?.members, let senderID = self.user?.senderID {
            for item in members {
                if (messageByUser == item.receiverID) && (item.receiverID != senderID) {
                    if let receiverImageUrl = item.receiverImage, receiverImageUrl != "" {
                        ...
                        var fName = item.receiverFirstName ?? ""
                        var mName = item.receiverMiddleName ?? ""
                        var lName = item.receiverLastName ?? ""
                        fName = fName != "" ? fName + " " : ""
                        mName = mName != "" ? mName + " " : ""
                        cell.nameLabel.text = fName + mName + lName
                    }
                }
            }
        }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if let text = message.message {
            height = estimateFrameForText(text: text).height + 55
...
extension ChatLogController_Group: UITextFieldDelegate {
    
}


// MARK:- UIImagePickerControlle extension
extension ChatLogController_Group: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension ChatLogController_Group: UITextViewDelegate {
