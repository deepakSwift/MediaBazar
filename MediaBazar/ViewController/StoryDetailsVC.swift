...
//  MediaBazar
//
//  Created by deepak Kumar on 06/04/20.
...
class StoryDetailsVC: UIViewController {
    ...
    var cellType:CellTypes = .collapseCell
//    var StoryDetails = MediaStroyModel()
    var StoryDetails = MediaStroyDocsModel()
...
//        docContainerView.isHidden = true
    ...
    func MediaDataSetup() {
        ...
        if StoryDetails.uploadTexts.count == 0 {
            textContainerView.isHidden = true
        } else {
            textContainerView.isHidden = false
        }
        
        if StoryDetails.uploadImages.count == 0 {
            imageContainerView.isHidden = true
        } else {
            imageContainerView.isHidden = false
        }
        
        if StoryDetails.uploadThumbnails.count == 0 {
            thumbnailContainerView.isHidden = true
        } else {
            thumbnailContainerView.isHidden = false
        }
        
        if StoryDetails.uploadAudios.count == 0 {
            audioContainerView.isHidden = true
        } else {
            audioContainerView.isHidden = false
        }
        
        if StoryDetails.uploadVideos.count == 0 {
            videoContainerView.isHidden = true
        } else {
            videoContainerView.isHidden = false
        }
        
        if StoryDetails.supportingDocs.count == 0 {
            docContainerView.isHidden = true
        } else {
...
// -----TableView------
extension StoryDetailsVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == tableViewEvenDetails {
            if indexPath.row == 0 {
                ...
                cell.buttonBuyNow.addTarget(self, action: #selector(buttonBuyNow(sender:)), for: .touchUpInside)
    ...
    @objc func buttonBuyNow(sender: UIButton){
        //let data = allStoryList.docs[sender.tag]
        if storyCategory == "Auction" {
            //            NKToastHelper.sharedInstance.showAlert(self, title: warningMessage.title, message: "Coming soon...")
            
            let bidVC = AppStoryboard.Stories.viewController(BiddingPricePopUp.self)

            bidVC.currency = currency
            bidVC.lastBiddeingPrice = lastBiddingPrice
            bidVC.realbiddingPrice = realBiddingPrice
            bidVC.auctionBiddingPrice = auctionBiddingPrice
            bidVC.storyId = self.storyId
//            bidVC.delegate = self
            bidVC.view.backgroundColor = UIColor.black.withAlphaComponent(0.8)
            bidVC.modalPresentationStyle = .overFullScreen
            self.present(bidVC,animated:true,completion:nil)
            
        } else {
            let buyNowVC = AppStoryboard.Stories.viewController(BuySharedEventVC.self)
            buyNowVC.storyCategory = self.storyCategory
            buyNowVC.time = self.time
            buyNowVC.categoryType = self.categoryType
            buyNowVC.biddingPrice = self.auctionBiddingPrice
            buyNowVC.desc = self.descriptions
            buyNowVC.price = self.price
            buyNowVC.headline = self.headline
            buyNowVC.storyId = self.storyId
...
// MARK:- Chat Method
extension StoryDetailsVC: AccessEventDetailCell {
...
extension StoryDetailsVC: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if collectionView == collectionViewImages {
            return StoryDetails.uploadImages.count
        } else if collectionView == collectionViewThumbImage {
            return StoryDetails.uploadThumbnails.count
        } else if collectionView == collectionViewText {
            return StoryDetails.uploadTexts.count
        } else if collectionView == colllectionViewAudio {
            return StoryDetails.uploadAudios.count
        } else if collectionView == collectionViewVideo {
            return StoryDetails.uploadVideos.count
        } else if collectionView == collectionViewSupportingFile {
            return StoryDetails.supportingDocs.count
        } else {
            return 0
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if collectionView == collectionViewImages {
            let imgUrl = StoryDetails.uploadImages[indexPath.item].Image
            ...
            let imgUrl = StoryDetails.uploadThumbnails[indexPath.item].thumbnale
            let getProfileUrl = "\(self.baseUrl)\(imgUrl)"//arrdata.journalistId.Image
            if let profileUrls = NSURL(string: (getProfileUrl)) {
                cell.imageViewSetImg.sd_setImage(with: profileUrls as URL, placeholderImage: #imageLiteral(resourceName: "user"))
                //cell.labelImageName.text = StoryDetails.uploadImages[indexPath.item].filename
