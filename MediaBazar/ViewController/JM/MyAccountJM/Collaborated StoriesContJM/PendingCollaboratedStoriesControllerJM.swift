//
//  PendingCollaboratedStoriesControllerJM.swift
//  MediaBazar
//
...
class PendingCollaboratedStoriesControllerJM: UIViewController {
    
    @IBOutlet weak var pendingcollabrateStoryTableView : UITableView!
    ...
    override func viewDidLoad() {
        ...
        setupTableView()
        getPendingStory(searchKey: "", header: currentUserLogin.token)
    ...
    func setupTableView(){
        self.pendingcollabrateStoryTableView.dataSource = self
        self.pendingcollabrateStoryTableView.delegate = self
        pendingcollabrateStoryTableView.reloadData()
    }
    ...
    func getPendingStory(searchKey: String, header: String ){
        Webservices.sharedInstance.getpendingCollobratedLists(searchkey: searchKey, header: header){(result,message,response) in
            print(result)
            if result == 200{
                if let somecategory = response{
                    self.myStorydata = somecategory
                    self.pendingcollabrateStoryTableView.reloadData()
                    self.calculateTime()
                    //                    self.setupData()
                    if self.myStorydata.docs.count == 0 {
                        //-----Showing label in case data not found
                        let noDataLabel: UILabel = UILabel(frame: CGRect(x: 0, y: 0, width: self.pendingcollabrateStoryTableView.bounds.size.width, height: self.pendingcollabrateStoryTableView.bounds.size.height))
                        noDataLabel.text = "No pending story available."
                        noDataLabel.textColor = UIColor.black
                        noDataLabel.textAlignment = .center
                        self.pendingcollabrateStoryTableView.backgroundView = noDataLabel
                        self.pendingcollabrateStoryTableView.backgroundColor = UIColor.white
                        self.pendingcollabrateStoryTableView.separatorStyle = .none
                    }
                } else{
    ...
    func getPendingSearchStory(searchKey: String, header: String ){
        Webservices.sharedInstance.getPendingCollobratedSearchLists(searchkey: searchKey, header:header){(result,message,response) in
            print(result)
            if result == 200{
                if let somecategory = response{
...
extension PendingCollaboratedStoriesControllerJM : UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PendingCollaboratedStoriesTableViewCellJM") as! PendingCollaboratedStoriesTableViewCellJM
        if isSearching{
...
extension PendingCollaboratedStoriesControllerJM: UISearchBarDelegate {
    //----------- SearchBar Delegate --------------
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        
        if searchBar.text != "" && searchBar.text != nil {
            isSearching = false
            getPendingSearchStory(searchKey: searchBar.text!, header: currentUserLogin.token)
        } else {
            isSearching = true
            getPendingStory(searchKey: "", header: currentUserLogin.token)
        }
