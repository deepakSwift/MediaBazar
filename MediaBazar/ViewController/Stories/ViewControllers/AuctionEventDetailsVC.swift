...
class AuctionEventDetailsVC: UIViewController {
    ...
    var auctionDetailData = storyListModal()
    var thumbnailImage = ""
    ...
    func setupData() {
        ...
        for data in auctionDetailData.uploadThumbnails.enumerated() {
            let tempData = data.element.thumbnale
            self.thumbnailImage = tempData
        }
    ...
    @objc func bidNowButtonPressed() {
        let popUpVC = self.storyboard?.instantiateViewController(withIdentifier: "BiddingPricePopUp") as! BiddingPricePopUp
//        popUpVC.delegate = self
    ...
    @objc func nameButtonPressed(){
        ...
        ProfileDetailVC.journalistIDByStory = auctionDetailData.journalistId.id
...
extension AuctionEventDetailsVC: UITableViewDataSource, UITableViewDelegate {
...
//                cell.buttonfree.backgroundColor = #colorLiteral(red: 0, green: 0.05098039216, blue: 0.6392156863, alpha: 1)
//                cell.buttonfree.setTitle("Auction", for: .normal)
...
//                cell.buttonBuyNow.addTarget(self, action: #selector(bidNowButtonPressed), for: .touchUpInside)
...
//                let arrdata = auctionDetailData.docs[indexPath.row]
                cell.labelProfileName.text = ("\(auctionDetailData.journalistId.firstName) \(auctionDetailData.journalistId.lastName)")
                cell.labelEventName.text = auctionDetailData.headLine
                cell.labelTechnology.text = auctionDetailData.storyCategory
                cell.labelTime.text = ("\(auctionDetailData.langCode) | \(auctionDetailData.createdAt) | \(auctionDetailData.country.name)")
                cell.labelPriceRete.text = String(auctionDetailData.price)
                cell.storyCategoryLabel.text = auctionDetailData.storyCategory
                ...
                cell.labelDescription.text = auctionDetailData.briefDescription
                
                cell.keyword = auctionDetailData.keywordName
...
extension AuctionEventDetailsVC: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
//extension AuctionEventDetailsVC: DismissPopUpDelagate {
//
//    func NavigateBuyAuction() {
//        let buyAuctionVC = self.storyboard?.instantiateViewController(withIdentifier: "BuyAuctionEventVC") as! BuyAuctionEventVC
//        self.navigationController?.pushViewController(buyAuctionVC, animated: true)
//    }
//}
