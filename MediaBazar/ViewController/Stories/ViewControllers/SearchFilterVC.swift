...
//  MediaBazar
//
//  Created by deepak Kumar on 02/04/20.
//  Copyright © 2020 Saurabh Chandra Bose. All rights reserved.
//

import UIKit

protocol SendNameOfAreaFilter {
...
class SearchFilterVC: UIViewController {
    ...
    var flag = false
    var delegate: SendNameOfAreaFilter!
    ...
    func setupData() {
        for countryName in areaOfInterestArea {
            Getdata.append(countryName.categoryText)
            GetDataId.append(countryName.categoryID)
        }
    }
    ...
    @IBAction func buttonActionClearFilter(_ sender: Any) {
        self.dismiss(animated: true) {
            self.delegate.FilterName(name: "clearFilter", id: "")
        }
        
    }
       
       func getCategory(){
           WebService3.sharedInstance.adminCategoryListData(){(result,response,message) in
...
//------ TableView -------
extension SearchFilterVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if flag == true {
            ...
            cell.labelText.text = areaOfInterestArea[indexPath.row].categoryText
        }
        return cell
    }
        ...
        self.dismiss(animated: true) {
            ...
            if self.flag == true {
                ...
                for cityName in self.areaOfInterestArea {
                    if CountryName == cityName.categoryText {
                        let countryId = cityName.categoryID
                        print("========\(CountryName)")
                        print("========\(countryId)")
                        self.delegate.FilterName(name: CountryName, id: countryId)
                    }
                }
            } else {
                let CountryName = self.areaOfInterestArea[indexPath.row].categoryText
                let countryId = self.areaOfInterestArea[indexPath.row].categoryID
                print("========\(CountryName)")
                print("========\(countryId)")
                self.delegate.FilterName(name: CountryName, id: countryId)
            }
