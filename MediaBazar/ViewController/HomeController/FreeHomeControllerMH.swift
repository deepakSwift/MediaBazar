...
class FreeHomeControllerMH: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        getAllStoryData(page: "0", key: "Free", header: self.currenUserLogin.mediahouseToken)
    }
        ...
        WebService3.sharedInstance.getStoryList(page: page, key: key, storyHeader: header){(result,message,response) in
            ...
            if result == 200{
                ...
                if self.storyList.count == 0 {
                    ...
                    let noDataLabel: UILabel = UILabel(frame: CGRect(x: 0, y: 0, width: self.freeHomeTableView.bounds.size.width, height: self.freeHomeTableView.bounds.size.height))
                    noDataLabel.text = "No story available."
                    noDataLabel.textColor = UIColor.black
                    noDataLabel.textAlignment = .center
                    self.freeHomeTableView.backgroundView = noDataLabel
                    self.freeHomeTableView.backgroundColor = UIColor.white
                    self.freeHomeTableView.separatorStyle = .none
                }
            }else{
        ...
        WebService3.sharedInstance.AddToFavoriteStroy(storyId: storyId, storyHeader: header) { (result, message, response) in
            ...
            if result == 200{
                ...
                if self.isSearching == true {
                    self.getAllStoryData(page: "0", key: "Free", header: self.currenUserLogin.mediahouseToken)
                }else {
                    self.getSearchData(page: "0", key: "Free", searchKey: self.searchText, header: self.currenUserLogin.mediahouseToken)
                }
            }else{
                if self.isSearching == true {
                    self.getAllStoryData(page: "0", key: "Free", header: self.currenUserLogin.mediahouseToken)
...
extension FreeHomeControllerMH: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "FreeHomeTableViewCell") as! FreeHomeTableViewCell
        if isSearching {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if (totalPaginationPage - 3) == indexPath.row {
            ...
            if self.typeOfData == "free"{
                ...
                getSearchData(page: "\(page)", key: "", searchKey: searchText, header: currenUserLogin.mediahouseToken)
            }else {
                return
            }
...
extension FreeHomeControllerMH: UISearchBarDelegate {
    //----------- SearchBar Delegate --------------
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        if searchBar.text != "" && searchBar.text != nil {
            ...
            self.page = 0
            self.totalPages = 0
            getSearchData(page: "0", key: "Free", searchKey: searchBar.text!, header: self.currenUserLogin.mediahouseToken)
        } else {
            isSearching = true
            self.storyList.removeAll()
            self.page = 0
...
//------getting Filter data
extension FreeHomeControllerMH: SendNameOfAreaFilter {
    func FilterName(name: String, id: String) {
        if name == "clearFilter" {
            ...
            self.getFilterID = id
            self.getFilterStoryData(page: "0", key: "Free", categoryId: id, header: self.currenUserLogin.mediahouseToken)
        }
    }
}
