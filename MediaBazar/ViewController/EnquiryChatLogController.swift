//
//  EnquiryChatLogController.swift
//  MediaBazar
//
...
class EnquiryChatLogController: UIViewController {
    ...
    func observeMessages() {
        ...
        let messageIdRef = Database.database().reference().child("chat").child("ethic").child(chatID)
    ...
    func observeOnlineStatus() {
        let statusOnceRef = Database.database().reference().child("ethic").child((user?.receiverID)!)
        ...
        let statusRef = Database.database().reference().child("ethic").child((user?.receiverID)!)
    ...
    @objc func handleSendButton() {
        ...
        if firstMessage {
            ...
            let messageIdRef = Database.database().reference().child("chat").child("ethic")
        ...
        childRef.updateChildValues(values) { (error, ref) in
            ...
            if self.tokenID != "" {
                //                self.chatInsert(header: self.tokenID, editorialBoardID: toID, message: self.inputTextField.text ?? "", messageType: "text")
                
                if self.user?.senderUserType == "journalist"{
                    self.insertJournalistChat(ethicsCommitedID: toID, message: self.inputTextField.text, messageType: "text", header: self.tokenID)
                }else if self.user?.senderUserType == "mediahouse"{
                    self.insertMediaListChat(ethicsCommitedID: toID, message: self.inputTextField.text, messageType: self.inputTextField.text, header: self.tokenID)
                }
            }
    ...
    func insertJournalistChat(ethicsCommitedID : String, message : String, messageType : String,header : String){
        Webservices.sharedInstance.insertJournalistEthicsChatList(ethicsCommitteId: ethicsCommitedID, message: message, messageType: messageType, header: header){(result,message,response) in
    ...
    func insertMediaListChat(ethicsCommitedID : String, message : String, messageType : String,header : String){
        WebService3.sharedInstance.insertMediaEthicsChatList(ethicsCommitteId: ethicsCommitedID, message: message, messageType: messageType, header: header){(result,message,response) in
            print(result)
            CommonClass.hideLoader()
            if result == 200{
            }else{
...
extension EnquiryChatLogController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDelegate {
...
extension EnquiryChatLogController: UITextFieldDelegate {
    
}


// MARK:- UIImagePickerControlle extension
extension EnquiryChatLogController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension EnquiryChatLogController: UITextViewDelegate {
