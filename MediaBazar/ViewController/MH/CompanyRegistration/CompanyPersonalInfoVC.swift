...
class CompanyPersonalInfoVC: UIViewController,UITextViewDelegate {
...
//        typePickerView.delegate = self
        ...
        textViewShortBio.delegate = self
    ...
    func setupButton(){
        ...
        func updateCharacterCount() {
            let summaryCount = self.textViewShortBio.text.count
         ...
         func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool{
            if(textView == textView){
               return textViewShortBio.text.count +  (text.count - range.length) <= 200
            }
...
//        self.navigationController?.pushViewController(personalInfo2VC, animated: true)
        if isValidate() {
            ...
            personalInfo(profileImageUrl: profileImageView.image!,organisationName: textFiledOrganisationName.text!, mediahouseTypeId: mediaTypeId, emailId: textFieldEmail.text!, mobileNumber: textFieldMobileNumber.text!, firstName: TextFiledFirstName.text!, middleName: TextFiledMiddleName.text!, lastName: TextFiledLastName.text!, designationId: self.designationId, langCode: "en", passcode: textFieldConfirmPassword.text!, currency: currencyCode, pinCode: textFieldZipCode.text!, shortBio: textViewShortBio.text!, mailingAdress: textViewmailingAddress.text!, Country: countryData, state: stateData, city: cityData, stepCount: "1", phoneCode: phoneCode)
        }
...
//        self.countryNamelabel.text = selectedCountry.phoneCode
    ...
    @IBAction func iconCreatePass(sender: AnyObject) {
        if(iconClick == true) {
            textFieldCreatePassword.isSecureTextEntry = false
            createPassIconButton.setImage(#imageLiteral(resourceName: "Group 3238"), for: .normal)
            
        } else {
            textFieldCreatePassword.isSecureTextEntry = true
            createPassIconButton.setImage(#imageLiteral(resourceName: "invisible"), for: .normal)
        }
        
        iconClick = !iconClick
    }
    
    @IBAction func iconConfirmPass(sender: AnyObject) {
        if(iconClick == true) {
            textFieldConfirmPassword.isSecureTextEntry = false
            confirmPassIconButton.setImage(#imageLiteral(resourceName: "Group 3238"), for: .normal)
            
        } else {
            textFieldConfirmPassword.isSecureTextEntry = true
            confirmPassIconButton.setImage(#imageLiteral(resourceName: "invisible"), for: .normal)
        }
...
//        }
        ...
        else if textFieldCreatePassword.text == "" {
            NKToastHelper.sharedInstance.showErrorAlert(self, message: "Please enter the password.")
            return false
        }
        else if !(textFieldCreatePassword.text?.isValidPassword)! {
        ...
        else if textFieldConfirmPassword.text != textFieldCreatePassword.text {
    ...
    //------ Call Api ----
    func personalInfo(profileImageUrl: UIImage, organisationName: String, mediahouseTypeId: String, emailId: String, mobileNumber: String, firstName:String, middleName: String, lastName: String, designationId: String, langCode: String, passcode: String, currency: String, pinCode: String, shortBio: String,mailingAdress : String, Country: String, state: String, city: String, stepCount: String, phoneCode: String ) {
        
        WebService3.sharedInstance.getPersonalInfo(profileImageUrl: profileImageUrl, organisationName: organisationName,mediahouseTypeId: mediahouseTypeId, emailId: emailId, mobileNumber: mobileNumber, firstName: firstName, middleName: middleName, lastName: lastName, designationId: designationId, langCode: langCode, passcode: passcode, currency: currency, pinCode: pinCode, shortBio: shortBio, mailingAddress: mailingAdress, Country: Country, state: state, city: city, stepCount: stepCount,phoneCode: phoneCode){(result,response,message)  in
...
extension CompanyPersonalInfoVC:CountryPickerViewDataSource, CountryPickerViewDelegate {
...
//------- TextField delegate ------
extension CompanyPersonalInfoVC: UITextFieldDelegate {
...
extension CompanyPersonalInfoVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
//------ getMediaType -------
extension CompanyPersonalInfoVC: SendNameOfMediaType {
...
extension CompanyPersonalInfoVC: SendNameOfCountry {
...
extension CompanyPersonalInfoVC: SendNameOfState {
...
extension CompanyPersonalInfoVC: SendNameOfCity {
...
extension CompanyPersonalInfoVC: SendNameOfCurrency {
