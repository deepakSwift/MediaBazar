...
protocol SendNameOfJobCategory {
    func jobCategoryName(name: String, id: String)
}

class JobCategorySearchVC: UIViewController {

    @IBOutlet weak var searchBarView: UISearchBar!
    @IBOutlet weak var tableViewCountry: UITableView!
    
    var flag = false
    var delegate: SendNameOfJobCategory!
    ...
    func doInitialSetup(){
       ...
       func setupData() {
           for countryName in areaOfInterestArea {
               Getdata.append(countryName.jobCategoryName)
       ...
       @IBAction func buttonActionDismiss(_ sender: Any) {
              self.dismiss(animated: true, completion: {
                self.delegate.jobCategoryName(name: "", id: "")
              })
          }
          
    func getCategory(){
        WebService3.sharedInstance.jobCategoryListData(){(result,response,message) in
...
//------ TableView -------
extension JobCategorySearchVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if flag == true {
            ...
            cell.labelText.text = areaOfInterestArea[indexPath.row].jobCategoryName
        }
        return cell
    }
        ...
        self.dismiss(animated: true) {
            ...
            if self.flag == true {
                ...
                for cityName in self.areaOfInterestArea {
                    if CountryName == cityName.jobCategoryName {
                        let countryId = cityName.keywordID
                        print("========\(CountryName)")
                        print("========\(countryId)")
                        self.delegate.jobCategoryName(name: CountryName, id: countryId)
                    }
                }
            } else {
                let CountryName = self.areaOfInterestArea[indexPath.row].jobCategoryName
...
extension JobCategorySearchVC: UISearchBarDelegate {
