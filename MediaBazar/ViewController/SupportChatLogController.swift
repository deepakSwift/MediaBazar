//
//  SupportChatLogController.swift
//  MediaBazar
//
...
class SupportChatLogController: UIViewController {
    
    lazy var collectionView: UICollectionView = {
        let flowLayout = UICollectionViewFlowLayout()
        
        let cv = UICollectionView(frame: self.view.bounds, collectionViewLayout: flowLayout)
        cv.register(ChatMessageCell_Support.self, forCellWithReuseIdentifier: "collectionCell")
    ...
    func observeMessages() {
        ...
        let messageIdRef = Database.database().reference().child("chat").child("admin").child(chatID)
    ...
    func observeOnlineStatus() {
        let statusOnceRef = Database.database().reference().child("admin").child((user?.receiverID)!)
        ...
        let statusRef = Database.database().reference().child("admin").child((user?.receiverID)!)
    ...
    @objc func handleSendButton() {
        ...
        if firstMessage {
            ...
            let messageIdRef = Database.database().reference().child("chat").child("admin")
...
//                self.chatInsert(header: self.tokenID, editorialBoardID: toID, message: self.inputTextField.text ?? "", messageType: "text")
    ...
    func chatInsert(header: String, editorialBoardID: String, message: String, messageType: String) {
        WebService3.sharedInstance.editorialChatInsert(header: header, editorialBoardID: editorialBoardID, message: message, messageType: messageType) { (statusCode, responseMessage) in
...
extension SupportChatLogController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "collectionCell", for: indexPath as IndexPath) as! ChatMessageCell_Support
    ...
    fileprivate func setupCell(_ cell: ChatMessageCell_Support, message: Message) {
...
extension SupportChatLogController: UITextFieldDelegate {
    
}


// MARK:- UIImagePickerControlle extension
extension SupportChatLogController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension SupportChatLogController: UITextViewDelegate {
