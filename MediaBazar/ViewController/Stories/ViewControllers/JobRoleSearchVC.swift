...
protocol SendNameOfJobRole {
    func jobRoleName(name: String, id: String)
}

class JobRoleSearchVC: UIViewController {

    @IBOutlet weak var searchBarView: UISearchBar!
      @IBOutlet weak var tableViewCountry: UITableView!
      
      var flag = false
      var delegate: SendNameOfJobRole!
    ...
    func doInitialSetup(){
        ...
        func apiCall() {
            ...
            getCategory(searchKey: "")
        }
        
        func setupData() {
            for countryName in areaOfInterestArea {
                Getdata.append(countryName.jobRoleName)
        ...
        @IBAction func buttonActionDismiss(_ sender: Any) {
               self.dismiss(animated: true, completion: {
                self.delegate.jobRoleName(name: "", id: "")
               })
           }
           
    func getCategory(searchKey: String){
        WebService3.sharedInstance.JobRoleListData(searchKey: searchKey){(result,response,message) in
...
//------ TableView -------
extension JobRoleSearchVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if flag == true {
            ...
            cell.labelText.text = areaOfInterestArea[indexPath.row].jobRoleName
        }
        return cell
    }
        ...
        self.dismiss(animated: true) {
            ...
            if self.flag == true {
                ...
                for cityName in self.areaOfInterestArea {
                    if CountryName == cityName.jobRoleName {
                        let countryId = cityName.keywordID
                        print("========\(CountryName)")
                        print("========\(countryId)")
                        self.delegate.jobRoleName(name: CountryName, id: countryId)
                    }
                }
            } else {
                let CountryName = self.areaOfInterestArea[indexPath.row].jobRoleName
...
extension JobRoleSearchVC: UISearchBarDelegate {
