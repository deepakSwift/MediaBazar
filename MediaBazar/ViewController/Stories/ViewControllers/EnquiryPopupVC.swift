//
//  EnquiryPopupVC.swift
//  MediaBazar
//
//  Created by Sagar Gupta on 17/02/20.
//  Copyright © 2020 Saurabh Chandra Bose. All rights reserved.
//

import UIKit

class EnquiryPopupVC: UIViewController {
    ...
    @objc func submitEnquiry(){
        if fromVC == "MediaHouseSetting" {
            if self.isValidate() {
                getMediaEnquiryPost(enquiryTitle: titleTxt.text!, header: currentUserLogin.mediahouseToken, enquiryDescription: briefDescriptionTextField.text!)
            }
        } else {
            if self.isValidate() {
    ...
    func getMediaEnquiryPost(enquiryTitle: String,header: String, enquiryDescription: String){
        CommonClass.showLoader()
    WebService3.sharedInstance.postMediaEnquiryData(enquiryTitle: enquiryTitle, enquiryDescription: enquiryTitle, header: header){(result,response,message) in
            CommonClass.hideLoader()
            print(result)
            if result == 200{
                if let somecategory = response{
