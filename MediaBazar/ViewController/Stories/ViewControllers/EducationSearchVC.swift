//
//  JobQualificationSearchVC.swift
//  MediaBazar
//
...
protocol SendNameOfJobQualification {
    func JobQualificationName(name: String, id: String)
}

class EducationSearchVC: UIViewController {
    
    @IBOutlet weak var searchBarView: UISearchBar!
    @IBOutlet weak var tableViewCountry: UITableView!
    
    var flag = false
    var delegate: SendNameOfJobQualification!
    ...
    func setupData() {
        for countryName in areaOfInterestArea {
            Getdata.append(countryName.jobQualificationName)
    ...
    @IBAction func buttonActionDismiss(_ sender: Any) {
        self.dismiss(animated: true, completion: {
            self.delegate.JobQualificationName(name: "", id: "")
        })
    }
          
    func getCategory(){
        WebService3.sharedInstance.JobQualificationData(){(result,response,message) in
...
//------ TableView -------
extension EducationSearchVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if flag == true {
            ...
            cell.labelText.text = areaOfInterestArea[indexPath.row].jobQualificationName
        }
        return cell
    }
        ...
        self.dismiss(animated: true) {
            ...
            if self.flag == true {
                ...
                for cityName in self.areaOfInterestArea {
                    if CountryName == cityName.jobQualificationName {
                        let countryId = cityName.keywordID
                        print("========\(CountryName)")
                        print("========\(countryId)")
                        self.delegate.JobQualificationName(name: CountryName, id: countryId)
                    }
                }
            } else {
                let CountryName = self.areaOfInterestArea[indexPath.row].jobQualificationName
...
extension EducationSearchVC: UISearchBarDelegate {
