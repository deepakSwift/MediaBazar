...
protocol sendVideoNote{
    func videoNote(note: String)
}


class VideoNoteViewController: UIViewController {
    ...
    @objc func clickOnsunbmitButton(){
        if showNote == "note"{
            ...
            self.dismiss(animated: true, completion: {
                self.delegate.videoNote(note: self.textView.text)
            })
        }
